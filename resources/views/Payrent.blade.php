                    ...
                    <div class="invoice">
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>2</td>
                                    <td>ค่าน้ำ {{ $billing->billing_month }}{{ $billing->billing_year }} ({{ $billing->water_units }} หน่วย)</td>
                                    <td>{{ $billing->water_charge }} บาท</td>
                                </tr>
                                <tr>
                                    <td>3</td>
                                    <td>ค่าไฟ  {{ $billing->billing_month }}{{ $billing->billing_year }} ({{ $billing->electric_units }} หน่วย)</td>
                        ...
                        @else
                        <form action="{{ route('uploadSlip', $billing->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <label for="billing_slip">อัปโหลดสลิปการชำระเงิน:</label>
                            <input type="file" name="billing_slip" id="billing_slip" accept="image/*" required>
                            <button type="submit" class="pay-now" id="pay-now-button" disabled>ยืนยันการชำระเงิน</button>
                        </form>
                        @endif
